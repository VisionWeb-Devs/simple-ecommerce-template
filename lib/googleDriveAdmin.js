...
const serviceAccount = {
  ...
  project_id: process.env.ADMIN_SERVICE_ACCOUNT_PROJECT_ID,
...
async function upload_file(formData) {
  ...
  let existingFolder = await findFolderByName(productURL);
  if (!existingFolder) {
...
async function change_image_variation(formData) {
  ...
  const variation = formData.get("variation");
  const index = formData.get("index");
  const parent_folder = await findFolderByName(productURL);
  ...
  if (image_file) {
    ...
    console.log(`File renamed to ${res.data.name}`);
  } else {
...
async function truncate_collection(collection) {
  let snapshot = await getDocs(collection(db, collection));
  snapshot.docs.map(async (docc) => {
    const ref = doc(db, collection, docc.id);
    await deleteDoc(ref);
  });
}
async function add_product(formData) {
  ...
  try {
    const docRef = doc(db, "products", data.productURL);
...
export { upload_file, add_product, change_image_variation };
